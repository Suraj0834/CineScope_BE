 ...
 * @desc    Generate AI summary/explanation for a movie using Google Gemini
...
router.post('/summarize', authenticateToken, async (req, res) => {
  try {
    ...
    // Build context for better AI response
 ...
 * @desc    Get detailed explanation about specific movie aspects using Gemini
 * @access  Private (JWT required)
 */
router.post('/explain', authenticateToken, async (req, res) => {
  try {
    ...
    console.error('Gemini explain error:', error.response?.data || error.message);
 ...
 * @desc    Get AI-powered movie recommendations based on preferences using Gemini
 * @access  Private (JWT required)
 */
router.post('/recommend', authenticateToken, async (req, res) => {
  try {
    ...
    const recommendations = response.data.candidates[0].content.parts[0].text.trim();
    ...
    console.error('Gemini recommend error:', error.response?.data || error.message);
 ...
 * @desc    Analyze movie themes, symbolism, and deeper meanings using Gemini
 * @access  Private (JWT required)
 */
router.post('/analyze', authenticateToken, async (req, res) => {
  try {
    ...
    const aspectType = aspect || 'themes and symbolism';
    
    let prompt = `Analyze the movie "${title}" focusing on ${aspectType}.`;
    if (plot) prompt += `\n\nPlot context: ${plot}`;
 ...
 * @desc    Chat with AI about movie rumors, myths, facts, and trivia
 * @access  Private (JWT required)
 */
...
User Question: ${userMessage}`;
    ...
    if (recRegex.test(userMessage)) {
      ...
      const recText = recResponse.data?.candidates?.[0]?.content?.parts?.[0]?.text;
      if (!recText) throw new Error('No recommendation response from AI');
      ...
      for (let line of lines) {
        ...
        if (m) {
          ...
          // Try to fetch details from OMDb
          try {
            const omdbResp = await axios.get(OMDB_BASE_URL, {
              params: {
                apikey: OMDB_API_KEY,
                t: titlePart
              }
